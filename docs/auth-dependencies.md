...
- No enabling of admin proxy. ADMIN_PROXY_ENABLED remains false. NEXT_PUBLIC_ADMIN_DEMO_ENABLED
  controls admin UI visibility only and is not changed by this milestone.
...
| NEXT_PUBLIC_ADMIN_DEMO_ENABLED  | Frontend (public)     | Controls admin UI visibility only. Does not grant API access or enable the admin proxy. May be true in a controlled demo environment. | false (default); true on the live Vercel demo | false (default)                  | Does not grant API access. Confusing this with ADMIN_PROXY_ENABLED is the primary risk.          |
| ADMIN_PROXY_ENABLED             | Backend (server-only) | Server-side switch that enables the admin proxy to forward requests to the backend using the server-held bearer token. Must remain false on all public deployments until real Supabase Auth session validation, real membership lookup, and production CSRF are complete. | false | false until all 4S.85G controls are fully implemented and verified | Setting true without complete hardening gives public visitors full admin API access via the server-held bearer token. |
