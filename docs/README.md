...
| [Trusted-user final access runbook (4S.99D)](4s99d-trusted-user-final-access-runbook.md) | Final pre-access runbook for 3–5 named trusted users only; covers final technical proof sequence, go/no-go decision record, access removal and rollback plan, and a final warning before any trusted-user access is granted. PRE-ACCESS ONLY — no access has been granted, no users created, no environment flags changed, no public pilot auth or admin proxy enabled, and no production/live operational approval given. Not legal advice, not a DPIA, not regulatory approval, not CQC approval, and not production readiness confirmation |
